 ...
 *
 * 모든 텍스트는 결정론적 규칙으로 생성된다. **...** 구간은 UI 강조 마킹.
 * names를 주면 문장에 실제 이름이 대입된다 (조사 자동 처리).
 */
import { STEM_ELEMENT, STEM_HANGUL, type Pillar, type SajuChart } from './manse';
import { getSipsin, getGunghap, SIPSIN_MEANING, type GunghapNames, type Sipsin } from './gunghap';
...
const CONFLICT_PATTERNS: Array<{
...
}> = [
  {
    ...
    text: '두 사람의 갈등 축은 **자존심**이에요. 서로 대등한 구도라 평소엔 제일 편한 사이지만, 싸움이 나면 먼저 굽히는 쪽이 지는 것 같아서 냉전이 길어져요. 화해의 기술: **잘잘못 정리는 미루고 "밥 먹자" 한마디로 끊어내기.** 이 조합은 논리로 푸는 것보다 일상으로 복귀하는 게 빨라요.',
  },
  {
    match: (ab, ba) => (ab === '편관' || ab === '정관') && (ba === '편재' || ba === '정재'),
    text: '갈등의 축은 **주도권과 소유**예요. 한쪽은 상대 앞에서 묘하게 긴장하고(관), 다른 쪽은 상대를 자기 사람으로 확실히 하고 싶어해요(재). 평소엔 이 구도가 서로를 끌어당기는 힘인데, 지치면 "왜 나만 맞추지"와 "왜 내 맘대로 안 되지"가 충돌해요. **역할을 가끔 뒤집어보는 게 처방**이에요 — 늘 리드하던 쪽이 하루 완전히 따라가 보기.',
  },
  {
    match: (ab, ba) => ab === '상관' || ba === '상관',
    text: '갈등의 도화선은 **말**이에요. 이 조합엔 상관 기운이 끼어 있어서, 대화가 제일 재밌는 커플인 동시에 농담이 선을 넘는 순간 상처가 훅 깊어져요. 특히 장난 끝에 나온 진담 반 농담 반이 위험해요. 규칙 하나면 충분해요: **싸울 때 "아까 그 말"을 소환하지 않기.**',
  },
  {
    match: (ab, ba) => (ab === '편인' || ab === '정인') && (ba === '식신' || ba === '상관'),
    text: '갈등 축은 **돌봄의 방향**이에요. 한쪽은 품어주려 하고(인성) 다른 쪽은 표현하고 발산하려 해요(식상). 좋을 땐 완벽한 보호자-자유인 조합인데, 틀어지면 "애 취급하지 마"와 "왜 내 걱정을 몰라줘"로 부딪혀요. **보호가 통제가 되는 순간을 서로 알아채는 게 관건**이에요.',
  },
];

function buildConflict(a: SajuChart, b: SajuChart, n: GunghapNames): ReportSection {
  const g = getGunghap(a, b, n);
  ...
  if (negatives.length > 0) {
    ...
    paragraphs.push(
      `명식에서 가장 눈에 띄는 마찰 지점은 **'${worst.summary.split(' — ')[0]}'**이에요. ${worst.detail} 이 지점은 없앨 수 있는 게 아니라 다루는 거예요 — **패턴을 아는 것만으로 절반은 해결**이에요.`,
    );
  } else {
    paragraphs.push(
  ...
  if (pattern === undefined && negatives.length > 0) {
    paragraphs.push(
      `서로에게 ${g.sipsinAtoB}·${g.sipsinBtoA}인 관계라는 것도 힌트예요. ${SIPSIN_MEANING[g.sipsinAtoB]} **이 성질이 과해지는 순간이 곧 갈등의 시작점**이니, 그 직전에 브레이크를 잡으면 돼요.`,
    );
  }
...
function buildTiming(
  a: SajuChart, b: SajuChart, n: GunghapNames,
  baseYear: number, resolveYearBranch: YearBranchResolver,
): ReportSection {
  const paragraphs: string[] = [];
  ...
  for (const y of years) {
    ...
    let line: string;
    if (ja === 'hap' && jb === 'hap') {
      line = `${y}년 — **두 사람의 배우자궁이 동시에 합을 이루는 해**예요. 관계가 다음 단계로 넘어가기 가장 좋은 타이밍.`;
    } else if (ja === 'hap' || jb === 'hap') {
      const who = ja === 'hap' ? n.a : n.b;
      line = `${y}년 — **${who}의 인연운이 발동하는 해**예요. 이때 ${who} 쪽에서 마음의 움직임이 커져요.`;
    } else if (ja === 'chung' || jb === 'chung') {
      const who = ja === 'chung' ? n.a : n.b;
  ...
  if (bestYear !== null) {
    paragraphs.push(
      bestHits === 2
        ? `정리하면, **두 사람의 해는 ${bestYear}년**이에요. 중요한 결정(고백, 동거, 결혼 얘기)을 이 해에 얹으면 흐름을 타요.`
        : `정리하면, 향후 3년 중 **가장 기운이 움직이는 해는 ${bestYear}년**이에요.`,
    );
  } else {
    paragraphs.push(
      '향후 3년은 큰 파도 없이 잔잔한 흐름이에요. 타이밍의 도움이 없다는 건, 반대로 **언제 결정해도 불리하지 않다**는 뜻이기도 해요.',
    );
  }
...
function buildIntimacy(a: SajuChart, b: SajuChart, n: GunghapNames): ReportSection {
  const paragraphs: string[] = [];
  ...
  const hidNameA = STEM_HANGUL[HIDDEN_STEM[a.day.branch]];
  const hidNameB = STEM_HANGUL[HIDDEN_STEM[b.day.branch]];

  paragraphs.push(
    `사주에서 겉궁합은 일간(드러나는 나), 속마음 궁합은 일지 속에 숨은 글자(지장간)로 봐요. ${n.a}의 일지 ${a.day.branch} 속에는 ${hidNameA}(${HIDDEN_STEM[a.day.branch]}), ${n.b}의 일지 ${b.day.branch} 속에는 ${hidNameB}(${HIDDEN_STEM[b.day.branch]})${josa(hidNameB, '이', '가')} 숨어 있어요 — **이게 두 사람이 가장 가까워졌을 때 드러나는 진짜 속마음**이에요.`,
  );

  if (innerAtoB === outerAtoB) {
    paragraphs.push(
      `**겉과 속이 같은 커플**이에요. 드러나는 관계(${outerAtoB})와 속마음의 관계(${innerAtoB})가 일치해서, 가까워질수록 "역시 내가 알던 그 사람"이라는 안정감이 커져요. 반전은 없지만 배신감도 없는, 투명한 속마음 궁합이에요.`,
    );
  } else {
    paragraphs.push(
      `**겉과 속이 다른, 반전 있는 커플**이에요. 겉으로는 ${outerAtoB}의 관계인데, 속마음끼리는 ${innerAtoB}·${innerBtoA}${josa(innerBtoA, '으로', '로')} 만나요. ${SIPSIN_MEANING[innerAtoB]} — 가까워질수록 이 얼굴이 나와요. 겉만 보고 판단하면 서로를 반만 아는 거예요.`,
    );
  }
  ...
  if (tA.label === tB.label) {
    ...
    paragraphs.push(
      `속마음의 온도가 달라요. ${n.a}${josa(n.a, '은', '는')} **'${tA.label}'**(${tA.desc} 타입), ${n.b}${josa(n.b, '은', '는')} **'${tB.label}'**(${tB.desc} 타입)이에요. 애정의 크기가 아니라 표현 방식이 다른 것뿐인데, 이걸 모르면 "쟤는 날 덜 좋아하나"로 오해하기 쉬워요. **상대의 온도로 번역해서 읽는 연습**이 속마음 궁합의 전부예요.`,
    );
  }
...
export function getDeepReport(
  a: SajuChart, b: SajuChart, baseYear: number,
  resolveYearBranch: YearBranchResolver, names?: Partial<GunghapNames>,
): DeepReport {
  const n: GunghapNames = { a: names?.a || 'A', b: names?.b || 'B' };
  return {
    conflict: buildConflict(a, b, n),
    timing: buildTiming(a, b, n, baseYear, resolveYearBranch),
    intimacy: buildIntimacy(a, b, n),
  };
}
